...
def get_ratio(NH_data, RB_data, HM_data, baseline):
    return np.log(np.array(NH_data) / baseline), np.log(np.array(RB_data) / baseline), np.log(
        np.array(HM_data) / baseline)
...
plt.savefig("./" + dirname + "/" + dirname + "_HP_timeseries_practice.png")
