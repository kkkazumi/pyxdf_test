...
NH_SYS_r,RB_SYS_r,HM_SYS_r = get_ratio(NH_SYS,RB_SYS,HM_SYS,svr_baseline)
...
#extracted_NH = extract(rotated_NH,tNH_SYS)
extracted_NH = extract(np.vstack((NH_SYS_r,tNH_SYS)),tNH_SYS)
extracted_RB = extract(np.vstack((RB_SYS_r,tRB_SYS)),tRB_SYS)
extracted_HM = extract(np.vstack((HM_SYS_r,tHM_SYS)),tHM_SYS)
...
with open('./'+dirname+"/"+ dirname+'_y_SYSvaluesRB.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['y_values'])
    for value in extracted_RB[:,1]:
        writer.writerow([value])

with open('./'+dirname+"/"+ dirname+'_y_SYSvaluesHM.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['y_values'])
